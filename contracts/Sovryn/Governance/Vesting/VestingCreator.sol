...
contract VestingCreator is AdminRole {
	...
	constructor(address _token, address _vestingRegistryProxy) public {
		require(_token != address(0), "Token address invalid");
		require(_vestingRegistryProxy != address(0), "Vesting registry address invalid");

		token = IERC20(_token);
		vestingRegistryLogic = IVestingRegistryLogic(_vestingRegistryProxy);
	}

	/**
	 * @notice transfers Token tokens to given address
	...
	function isEnoughBalance() public view returns (bool) {
		return token.balanceOf(address(this)) >= getUnprocessedAmount();
	}
	...
	function getMissingBalance() external view returns (uint256) {
		...
		return getUnprocessedAmount() - token.balanceOf(address(this));
	}
