...
contract("Staking", (accounts) => {
	...
	let root, account1, account2, account3, governorVault;
	...
	before(async () => {
		[root, account1, account2, account3, governorVault, ...accounts] = accounts;
	});
	...
	beforeEach(async () => {
		...
		await staking.setFeeSharing(governorVault);
	...
	describe("withdraw", () => {
		...
		it("Should be able to withdraw second time", async () => {
			...
			let feeSharingBalance = await token.balanceOf.call(governorVault);
		...
		it("Should be able to withdraw earlier for any lock date", async () => {
			...
			for (let i = 1; i <= 78; i++) {
				...
				expect(feeSharingBalance).to.be.bignumber.equal(new BN(punishedAmount).add(earlierFeeSharingBalance));
