...
contract("OriginsBase (Verifier Functions)", (accounts) => {
	...
	it("Verifier should not be able to set deposit address.", async () => {
		await expectRevert(originsBase.setDepositAddress(newDepositAddr, { from: verifier }), "OriginsAdmin: Only owner can call this function.");
	});
	...
	it("Verifier should not be able to set Locked Fund Contract.", async () => {
		...
		await expectRevert(originsBase.setLockedFund(newLockedFund.address, { from: verifier }), "OriginsAdmin: Only owner can call this function.");
	});

    it("Verifier should not be able to add a new tier.", async () => {
        await token.mint(owner, firstRemainingTokens);
        await token.approve(originsBase.address, firstRemainingTokens, { from: owner });
        await expectRevert(originsBase.createTier(firstMaxAmount, firstRemainingTokens, firstSaleStartTS, firstSaleEnd, firstUnlockedBP, firstVestOrLockCliff, firstVestOfLockDuration, firstDepositRate, firstDepositType, firstVerificationType, firstSaleEndDurationOrTS, firstTransferType, { from: verifier }), "OriginsAdmin: Only owner can call this function.");
    });

    it("Verifier should not be able to set Tier Deposit Parameters.", async () => {
        await expectRevert(originsBase.setTierDeposit(tierCount, secondDepositRate, secondDepositToken, secondDepositType, { from: verifier }), "OriginsAdmin: Only owner can call this function.");
    });

    it("Verifier should not be able to set Tier Token Limit Parameters.", async () => {
        await expectRevert(originsBase.setTierTokenLimit(tierCount, secondMinAmount, secondMaxAmount, { from: verifier }), "OriginsAdmin: Only owner can call this function.");
    });

    it("Verifier should not be able to set Tier Token Amount Parameters.", async () => {
        await expectRevert(originsBase.setTierTokenAmount(tierCount, secondRemainingTokens, { from: verifier }), "OriginsAdmin: Only owner can call this function.");
    });

    it("Verifier should not be able to set Tier Vest or Lock Parameters.", async () => {
        await expectRevert(originsBase.setTierVestOrLock(tierCount, secondVestOrLockCliff, secondVestOfLockDuration, waitedTS, secondUnlockedBP, secondTransferType, { from: verifier }), "OriginsAdmin: Only owner can call this function.");
    });

    it("Verifier should not be able to set Tier Time Parameters.", async () => {
        await expectRevert(originsBase.setTierTime(tierCount, secondSaleStartTS, secondSaleEnd, secondSaleEndDurationOrTS, { from: verifier }), "OriginsAdmin: Only owner can call this function.");
    });
    ...
    it("Verifier should not be able to verify a zero address to a single tier.", async () => {
        await expectRevert(originsBase.addressVerification(zeroAddress, tierCount, { from: verifier }), "OriginsBase: Address to be verified cannot be zero.");
    });
    ...
    it("Verifier should not be able to verify a multiple address of length x to a multiple tier of length y, where x != y.", async () => {
        await expectRevert(originsBase.multipleAddressAndTierVerification([userOne, userTwo, userThree], [tierCount, tierCount-1], { from: verifier }), "OriginsBase: Address and Tier Array length mismatch.");
    });
