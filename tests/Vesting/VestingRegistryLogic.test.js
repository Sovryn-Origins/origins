...
const { mineBlock } = require("../Ethereum");
...
contract("VestingRegistryLogic", (accounts) => {
	let root, account1, account2, account3, account4, governorVault;
	let SOV, lockedFund;
	...
	before(async () => {
		[root, account1, account2, account3, accounts4, governorVault, ...accounts] = accounts;
		waitedTS = 1;
	});
	...
	describe("initialize", () => {
		it("fails if the 0 address is passed as vestingFactory address", async () => {
			await expectRevert(
				vesting.initialize(ZERO_ADDRESS, SOV.address, staking.address, governorVault, account1, lockedFund.address),
		...
		it("fails if the 0 address is passed as feeSharingProxy address", async () => {
			await expectRevert(
				vesting.initialize(vestingFactory.address, SOV.address, staking.address, ZERO_ADDRESS, account1, lockedFund.address),
		...
		it("fails if the 0 address is passed as LockedFund address", async () => {
			await expectRevert(
				vesting.initialize(vestingFactory.address, SOV.address, staking.address, governorVault, account1, ZERO_ADDRESS),
				"lockedFund address invalid"
			);
		});
	...
	describe("transferSOV", () => {
		it("should be able to transfer SOV", async () => {
			...
			let tx = await vesting.transferToken(account1, amount);
			expectEvent(tx, "TokenTransferred", {
				receiver: account1,
				amount: amount,
			});
		...
		it("only owner should be able to transfer", async () => {
			await expectRevert(vesting.transferToken(account1, 1000, { from: account1 }), "unauthorized");
		});

		it("fails if the 0 address is passed as receiver address", async () => {
			await expectRevert(vesting.transferToken(ZERO_ADDRESS, 1000), "receiver address invalid");
		});

		it("fails if the 0 is passed as an amount", async () => {
			await expectRevert(vesting.transferToken(account1, 0), "amount invalid");
		});
	});
	...
	describe("createVesting and getVesting - LockedFund", () => {
		it("Should create vesting and return the address for LockedFund", async () => {
			...
			let unlocked = 0;
			let vestingType = 1;
			await SOV.approve(lockedFund.address, amount);
			await lockedFund.depositVested(accounts4, amount, cliff, duration, basisPoint, unlocked, true);
			let vestingData = await lockedFund.getVestingData(cliff, duration);
			await lockedFund.createVesting(vestingData, { from: accounts4 });
			let vestingAddr = await vesting.getVestingAddr(accounts4, cliff * FOUR_WEEKS, duration * FOUR_WEEKS, vestingType);
