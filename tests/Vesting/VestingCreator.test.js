...
contract("VestingCreator", (accounts) => {
	let root, account1, account2, account3, account4, account5, governorVault;
	let SOV, lockedFund;
	...
	before(async () => {
		[root, account1, account2, account3, account4, account5, governorVault, ...accounts] = accounts;
		waitedTS = 1;
	});
	...
	describe("constructor", () => {
		it("sets the expected values", async () => {
			let _sov = await vestingCreator.token();
		...
		it("fails if the 0 address is passed as SOV Address", async () => {
			await expectRevert(VestingCreator.new(ZERO_ADDRESS, vesting.address), "Token address invalid");
		});
	...
	describe("transferSOV", () => {
		it("should be able to transfer SOV", async () => {
			...
			let tx = await vestingCreator.transferToken(account1, amount);
		...
		it("only owner should be able to transfer", async () => {
			await expectRevert(vestingCreator.transferToken(account1, 1000, { from: account1 }), "unauthorized");
		});

		it("fails if the 0 address is passed as receiver address", async () => {
			await expectRevert(vestingCreator.transferToken(ZERO_ADDRESS, 1000), "transfer to the zero address");
		});

		it("fails if the 0 is passed as an amount", async () => {
			await expectRevert(vestingCreator.transferToken(account1, 0), "amount invalid");
		});
	});
